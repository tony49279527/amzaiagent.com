...
async def send_email_report(report: AnalysisReport, is_pro_flow: bool = False):
    ...
    try:
        ...
        if is_pro_flow:
            ...
            # === FULL REPORT EMAIL (after balance paid or free-tier report) ===
            msg["Subject"] = f"Your Full AI Product Discovery Report: {report.keywords}"
            # Prefer report_html; fallback to markdown converted to HTML so body is complete and readable
            body_html = report.report_html if report.report_html else ""
            if not body_html and report.report_markdown:
                try:
                    import markdown
                    ...
                    print(f"Markdown-to-HTML fallback failed: {e}, using pre")
                    body_html = f"<pre>{report.report_markdown}</pre>"
            len_md = len(report.report_markdown or "")
            len_html = len(body_html)
            print(f"Full report email: markdown={len_md} chars, body_html={len_html} chars -> {report.user_email}")
            if len_md < 500:
                print(f"Warning: report content is very short ({len_md} chars). Check report generation or storage.")
            # Inline table styles for better email client rendering
            body_html = body_html.replace("<table>", '<table style="border-collapse:collapse;width:100%;margin:12px 0;">')
            body_html = body_html.replace("<th>", '<th style="border:1px solid #ddd;padding:8px;text-align:left;background:#f5f5f5;">')
            body_html = body_html.replace("<td>", '<td style="border:1px solid #ddd;padding:8px;">')
            html_content = (
                "<html><body style=\"font-family: Arial, sans-serif; line-height: 1.6; color: #333;\">"
                "<div style=\"max-width: 800px; margin: 0 auto; padding: 20px;\">"
                "<h2 style=\"color: #2563eb;\">Product Discovery Analysis Complete</h2>"
                "<p>Here is your full analysis report for <strong>"
                + (report.keywords or "").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                + "</strong> in <strong>"
                + (report.category or "").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
                + "</strong>.</p>"
